...
from model import DNN, train_model, get_model_predictions
import pytorch_data_loader 
#import data_loader 
from helper import get_data_loder_objects, get_model_names, create_dir, get_loader_kwargs, get_class_wise_accuracy, make_pickleable, save_objects, inverse_transpose_images
...
if __name__ == '__main__':
    ...
    if downsample_const is not None:
        ...
        for i in range(ds_obj_complete.num_classes()):
            ds_obj_mapping[i], datasets_mapping[i], data_loaders_mapping[i] = get_data_loder_objects("CIFAR10_{}".format(ds_obj_complete.classes[i]), PHASES, **get_loader_kwargs(batch_size))
    ...
    #         filename = '{}_CrossEntropyLoss_epoch_best_lr_{}.pth'.format(model.model_name, learning_rate)
    ...
    for model_name in get_model_names(dataset_name):
        for idx, model in enumerate(models_to_copies[model_name]):
            ...
            filename = '{}_CrossEntropyLoss_epoch_{}_lr_{}.pth'.format(model.model_name, epoch, learning_rate)
            print(filename)

            model.model_ft.load_state_dict(torch.load('../{}/model_weights/{}'.format(ds_obj.name, filename),
                                                    map_location=device))
            # model.model_ft.load_state_dict(torch.load('adience_classifier_epoch_best_lr_0.0005.pth',
            #                                         map_location=device))
